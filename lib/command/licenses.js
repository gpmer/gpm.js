...
const {normalizePath, isExistPath, isGitRepoDir, parseGitConfigAsync, parseLicense} = require('../utils');
...
function *licenses(key, options) {
  ...
  while (repositories.length) {
    ...
    let gitConfig = yield parseGitConfigAsync({cwd: repoDir, path: path.join('.git', 'config')});
  ...
  const basePath = path.join(config.paths.home, globalConfig.entity.base || config.defaults.base);

  const ROOT = normalizePath(basePath, options);

  !options.nolog && process.stdout.write(jt.treeString(output, {
      space: 4,
      vSpace: 0,
      valueOut: true,
