...
function *relink(argv, options) {
  ...
  if (_.isEmpty(sources)) {
    !options.quiet && logger.error(`You did't have any repository to relink, please add it first.`);
    return;
  }
  ...
  while (sources.length) {
    ...
    while (owners.length) {
      ...
      while (repos.length) {
        ...
        !options.quiet && logger.info(`${origin.url.green} >>> ${normalizePath(dist, options).yellow}`);
      }
    }
  }
