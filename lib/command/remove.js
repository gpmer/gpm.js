...
function *remove(key, options) {
  ...
  let choices = repositories.map(v => v.__index__);
  ...
  if (key) {
    ...
    choices = repositories.filter(function (repo) {
  ...
  const target = _.find(repositories, repo => {
  ...
  _.remove(repositories, repo => target.source === repo.source && target.owner === repo.owner && target.name === repo.name);
