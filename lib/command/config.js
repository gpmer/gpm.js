...
function *configHandler(argv, options) {
  ...
  switch (action.toUpperCase()) {
    case 'LIST':
      ...
      !options.nolog && process.stdout.write(prettyjson.render(globalConfig.entity) + '\n');
      break;
    case 'GET':
      if (!key) return !options.nolog && logger.error(`The [key] must be required`);
      output = globalConfig.get(key);
      !options.nolog && logger.info(`${key}: ${output}`);
      break;
    case 'SET':
      if (!key) return !options.nolog && logger.error(`The [key] must be required`);
      if (!value) return !options.nolog && logger.error(`The <value> must be required`);
    ...
    case 'RESET':
      ...
      break;
    default:
      !options.nolog && logger.info(`Flow the command line: ${(config.name + ' config ' + 'list|get|set|remove|reset'.yellow.underline + ' [key] [value]').green}`);
      break;
  }
  return output;
}
