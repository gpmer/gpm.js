...
function* get(key, configJSON) {
  ...
  logger.info(`${key}: ${(configJSON[key]).trim()}`);
}
...
function *configHandler(action, key, value, options) {
  ...
  switch (action.toUpperCase()) {
    ...
    case 'RESET':
      ...
      logger.info(`Has reset the config to default`);
      process.stdout.write(prettyjson.render(config.defaults) + '\n');
      break;
    default:
      logger.info(`Flow the command line: ${(config.name + ' config ' + 'list|get|set|delete|reset'.yellow.underline + ' [key] [value]').green}`);
      break;
  }
}

module.exports = function (argv, options) {
  return configHandler(argv.action, argv.key, argv.value, options);
};