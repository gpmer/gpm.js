...
function *add(repo, options) {
  ...
  const baseDir = path.join(config.paths.home, globalConfig.entity.base || config.defaults.base);
  ...
  if (gpmrc.exist) {
    ...
    if (alias && alias !== gitInfo.name) {
      let newRepoDir = path.join(ownerDir, _.isString(options.name) ? options.name : alias);
