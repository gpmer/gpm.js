...
const {isExistPath, isLink, normalizePath} = require('../utils');
...
function *add(repo, options) {
  ...
  if (yield isExistPath(tempDir)) {
    console.log('exist');
    yield fs.chmodAsync(tempDir, 777);
  ...
  const clone = spawn('git', ['clone', gitInfo.href], {cwd: config.paths.temp, stdio: 'inherit'});

  yield new Promise(function (resolve, reject) {
    clone.on('close', (code, signal) => {
      code === 0 ? resolve() : reject(new Error(`Error Code: ${code}, Exist Signal: ${signal}`));
    });
  });
  ...
  if (yield isExistPath(gpmrcPath)) {
    ...
    const cmd = hooks.add.split(' ');
    if (cmd.length) {
      yield new Promise(function (resolve, reject) {
        spawn(cmd.shift(), cmd, {cwd: repoDir, stdio: 'inherit'})
  ...
  process.chdir(cwd);
  !options.nolog && havePlugins && logger.info(`Plugin done`);
}

module.exports = function (argv, options) {
  pluginLoader.load(ACTION, options.plugin);
