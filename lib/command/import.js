...
const {isGitRepoDir, parseGitConfigAsync, spawnShell, isExistPath, isLink} = require('../utils');

function *importHandler(targetPath, options) {
  targetPath = path.resolve(process.cwd(), targetPath);
  const isGitDir = yield isGitRepoDir(targetPath);

  if (!isGitDir) return logger.error(`Invalid path: ${targetPath.green}, please make sure that is a git repository`);
  ...
  if (!origin.url) {
    return logger.error(`Invalid repository, please make sure that you have set remote repository url`);
  }
  ...
  logger.info(`${targetPath.green} has been linked to ${distPath.yellow}`);
}
