...
const {isGitRepoDir, parseGitConfigAsync, isExistPath, isLink, normalizePath} = require('../utils');

function *importHandlerOneDir(targetPath, options) {
  targetPath = path.resolve(process.cwd(), targetPath);
  const isGitDir = yield isGitRepoDir(targetPath);

  if (!isGitDir) throw new Error(`Invalid path: ${(normalizePath(targetPath, options)).green}, please make sure that is a git repository`);
  ...
  if (hasExistDist || hasLinkDist) {
    ...
    if (options.force) {
      ...
      isConfirmReplace = yield prompt({
        type: 'confirm',
        name: 'result',
        message: `repository has exist, Are you sure to replace ${normalizePath(distPath, options).yellow}`.white,
        "default": false,
      });
    }
  ...
  logger.info(`${normalizePath(targetPath, options).green} has been ${action} to ${normalizePath(distPath, options).yellow}`);
