...
function* importHandlerOneDir(targetPath, options) {
  ...
  if (!isGitDir)
    throw new Error(
      `Invalid path: ${normalizePath(targetPath, options).green}, please make sure that is a git repository`
    );
  ...
  if (!origin.url)
    throw new Error(
      `Invalid repository, please make sure that you have set remote repository url`
    );
  ...
  if (process.platform === 'win32') {
    logger.warn(
      `if you are using fucking <cmd>, Make sure you run it as administrator, or it will not have permission`
    );
  }
