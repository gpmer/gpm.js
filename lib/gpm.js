...
program
  ...
  .option(FLAGS.nolog.flag, FLAGS.nolog.desc)
  .action(bootstrapWrapper(function (argv, options) {
    co(require('./command/tree')(argv, options))
      .catch(errorHandler);
  }));
...
program
  .command('licenses')
  .alias('lc')
  .description('Give you the license (and URL to the source code) associated with each repository..')
  .option(FLAGS.unixify.flag, FLAGS.unixify.desc)
  .option(FLAGS.force.flag, FLAGS.force.desc)
  .option(FLAGS.nolog.flag, FLAGS.nolog.desc)
  .action(bootstrapWrapper(function (argv, options) {
    co(require('./command/licenses')(argv, options))
      .catch(errorHandler);
  }));
