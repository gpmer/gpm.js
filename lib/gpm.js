...
program
  .command('add')
  .alias('a')
  .argument('<repo>', 'git url address', /^(https|git)(.*)\.git$/)
...
program
  ...
  .alias('tr')
  .argument('[key]', 'filter by the key')
  .description('Display the all repositories in registry as a tree view')
  .option(FLAGS.unixify.flag, FLAGS.unixify.desc)
  .option(FLAGS.force.flag, FLAGS.force.desc)
  .option(FLAGS.nolog.flag, FLAGS.nolog.desc)
  .action(function(argv, options) {
...
program
  ...
  .alias('fd')
  .description(
    'Find repository by a key, You can get all about the repository info'
  )
  .option(FLAGS.unixify.flag, FLAGS.unixify.desc)
  .option(FLAGS.force.flag, FLAGS.force.desc)
  .option(FLAGS.nolog.flag, FLAGS.nolog.desc)
...
program
  ...
  .alias('rl')
  .description(
    'Relink the base directory and gpm registry, like Angular, trigger the $digest in manual'
  )
...
program
  ...
  .description('A series handler of configurations.')
  .option(FLAGS.unixify.flag, FLAGS.unixify.desc)
  .option(FLAGS.force.flag, FLAGS.force.desc)
  .option(FLAGS.nolog.flag, FLAGS.nolog.desc)
  .action(function(argv, options) {
...
program
  ...
  .description(
    'Run the plugin in each project. The current work directory is the current project.'
  )
  .option(FLAGS.unixify.flag, FLAGS.unixify.desc)
  .option(FLAGS.force.flag, FLAGS.force.desc)
  .option(FLAGS.nolog.flag, FLAGS.nolog.desc)
...
program
  ...
  .alias('lc')
  .description(
    'Give you the license (and URL to the source code) associated with each repository..'
  )
  .option(FLAGS.unixify.flag, FLAGS.unixify.desc)
  .option(FLAGS.force.flag, FLAGS.force.desc)
  .option(FLAGS.nolog.flag, FLAGS.nolog.desc)
