...
program
  ...
  .option("-u, --unixify", "output the path as unix style, useful in Windows Git bash or Power shell")
...
program
  ...
  .action(bootstrapWrapper(function (key, options) {
    process.stderr.write('WARNING'.yellow + ':search <key> has beem @Deprecated, Please use find instead. it will be remove in next major version\n');
...
program
  ...
  .action(bootstrapWrapper(function (options) {
    co(require('./command/search')(options)).catch(errorHandler);
  }));
